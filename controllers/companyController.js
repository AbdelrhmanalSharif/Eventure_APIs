...
const createCompanyEvent = async (req, res) => {
  try {
    ...
    if (!title || !description || !categories || !Array.isArray(categories) || categories.length === 0 || !location || !startDate || !endDate || !currency) {
...
const updateCompanyEvent = async (req, res) => {
  try {
    ...
    try {
      ...
      if (maxAttendees !== undefined) updateFields.push(`MaxAttendees = @maxAttendees`);

      const updateQuery = `UPDATE Events SET ${updateFields.join(', ')} WHERE EventID = @eventId`;
      const eventRequest = new sql.Request(transaction).input('eventId', sql.Int, eventId);
      if (title) eventRequest.input('title', sql.NVarChar, title);
      if (description) eventRequest.input('description', sql.NVarChar, description);
      if (location) eventRequest.input('location', sql.NVarChar, location);
      if (price !== undefined) eventRequest.input('price', sql.Decimal(10, 2), price);
      if (currency) eventRequest.input('currency', sql.NVarChar(10), currency);
      if (startDate) eventRequest.input('startDate', sql.DateTime, new Date(startDate));
      if (endDate) eventRequest.input('endDate', sql.DateTime, new Date(endDate));
      if (maxAttendees !== undefined) eventRequest.input('maxAttendees', sql.Int, maxAttendees);
