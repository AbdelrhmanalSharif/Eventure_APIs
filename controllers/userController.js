...
const registerUser = async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .input("profilePicture", sql.NVarChar, profilePicture).query(`
...
const loginUser = async (req, res) => {
  try {
    ...
    await pool
      ...
      .input("expiresAt", sql.DateTime, refreshTokenExpiry).query(`
...
const getUserProfile = async (req, res) => {
  try {
    ...
    const result = await pool.request().input("userId", sql.Int, userId).query(`
        SELECT UserID, FullName, Email, UserType, ProfilePicture, CreatedAt, PhoneNb
        FROM Users
        WHERE UserID = @userId
      `);
...
const updateUserProfile = async (req, res) => {
  try {
    const userId = req.user.userId;
    const { fullName, phoneNb } = req.body;
    const profilePicture = req.file
      ? `/uploads/users/${req.file.filename}`
      : null;
    ...
    await pool
      ...
      .input("fullName", sql.NVarChar, fullName)
      .input("phoneNb", sql.NVarChar, phoneNb)
      .input("profilePicture", sql.NVarChar, profilePicture).query(`
        UPDATE Users
        SET FullName = @fullName, ProfilePicture = @profilePicture, PhoneNb = @phoneNb
        WHERE UserID = @userId
      `);
...
const changePassword = async (req, res) => {
  try {
    ...
    if (newPassword.length < passwordConfig.minLength) {
      return res.status(400).json({
        message: `New password must be at least ${passwordConfig.minLength} characters long`,
      });
    }
    ...
    const currentHashedPassword =
      userResult.recordset[0].PasswordHash.toString();
