...
const getMyEvents = async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .query(
        "SELECT EventID FROM Events WHERE CompanyID = @userId ORDER BY CreatedAt DESC"
      );
...
const updateEvent = async (req, res) => {
  try {
    ...
    if (
      ...
      check.recordset[0].CompanyID !== userId
    ) {
      return res
        .status(403)
    ...
    if (maxAttendees !== undefined) {
      ...
      request.input("maxAttendees", sql.Int, maxAttendees);
    }

    if (updateFields.length > 0) {
      await request.query(
        `UPDATE Events SET ${updateFields.join(", ")} WHERE EventID = @eventId`
      );
    }
    ...
    if (latitude !== undefined && longitude !== undefined) {
      const mapCheck = await pool
        .request()
        .input("eventId", sql.Int, eventId)
        .query("SELECT * FROM MapsIntegration WHERE EventID = @eventId");
  ...
  try {
    ...
    await pool
      ...
      .input("eventId", sql.Int, eventId).query(`
        INSERT INTO Bookmarks (UserID, EventID)
...
const getUserBookmarks = async (req, res) => {
  try {
    ...
    const result = await pool.request().input("userId", sql.Int, userId).query(`
        SELECT EventID FROM Bookmarks WHERE UserID = @userId`);
    console.log(result.recordset);
    res.status(200).json(result.recordset.map((row) => row.EventID));
  } catch (error) {
